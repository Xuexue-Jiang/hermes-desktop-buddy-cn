...
function Welcome({
  ...
  async function handleConnectRemote(): Promise<void> {
    ...
    if (!url) {
      setRemoteError(t("welcome.remoteUrlRequired"));
      return;
    }
    ...
    try {
      ...
      if (ok) {
        ...
        setRemoteError(t("welcome.remoteUnreachable"));
      }
    } catch {
      setRemoteError(t("welcome.remoteConnectionFailed"));
    } finally {
  ...
  async function handleConnectSsh(): Promise<void> {
    ...
    if (!host || !user) {
      setSshError(t("welcome.sshRequired"));
      return;
    }
    ...
    try {
      ...
      if (ok) {
        ...
        setSshError(t("welcome.sshUnreachable"));
      }
    } catch (e) {
      setSshError(
        t("welcome.sshConnectionFailed", { message: (e as Error).message }),
      );
    } finally {
      setSshTesting(false);
    }
      ...
      <div className="screen welcome-screen">
        ...
        <div className="welcome-remote-card">
          ...
          <label className="welcome-remote-label" style={{ marginTop: 12 }}>
            ...
            <span style={{ fontWeight: 400, opacity: 0.6 }}>
              ({t("welcome.sshRemotePortDefault")})
            </span>
          </label>
          <input
            type="number"
          ...
          <p className="welcome-remote-hint">
            {t("welcome.sshHintBeforeCommand")}{" "}
            <code style={{ fontFamily: "monospace", fontSize: 12 }}>
              ssh {sshUser || "user"}@{sshHost || "host"}
            </code>{" "}
            {t("welcome.sshHintAfterCommand")}
          </p>
        </div>
